...
date: July 20th, 2020
bibliography: 'simpson-aquatic-virtual-summit.bib'
output:
...
Big Data presents us with challenges and through this talk today I want to use a small data case study to highlight where as a field I think we are missing the opportunity afford by big data because of the way limnologists often approach analysing those data
...
When we analyse data we mostly focus on this property of our data
...
B: in the low resilience state, where the basin of attraction has become shallower, the ecosystem takes lnoger to return to equilibrium following peturbation and hence exhibits high variance
...
Here the anlyst first detrends the time series
...
Move the window laong one time step and repeat 3 and 4 until you get to the end of the series

A trend in the resulting variance time series is estimated using Kendall's rank correlation tau

In this example the interest was in the end of the series so they right-aligned the window meaning they had to observ half the series before they could calculate the variance
...
* How to detrend (method, compexity of trend, ...)
* What window width?
...
The whole approach is *ad hoc* with many knobs to twiddle &mdash; how do you detrend the series? what width of windo should be used?
...
Today I want to illustrate how we can use modern statistical models to continously estimate the variance of multivariate time series using data from Lake 227 in the Experimental Lakes Area, Canada
...
Kathy Cottingham, Jim Rusak, and Peter Leavitt previously analysed these data by separating them into control and treated sections and compared the variances of the two periods with a Levene's test, euivalent of a t-test but for differences of variances not means
...
The gamma distribution would be more appropriate for describing the pigment data as it allows for positive continous values, where those values are skewed, with some large values
...
We assmue the responses are conditionally distributed gamma, and we model the mean and the shape parameter of the distribution with linear predictors
...
While we have made progress on the big-data side of things, we have made less progress as a field in how we analyse these big data once they have been collected or collated

In many respects, these are problems of small data too; unless as a field we rethink the way we analyse big limnological data we risk wasting the opportunity that these data affords us
...
4. Machine learning is *not* a panacae
...
And as I hopefully showed here, trends or response are often non-linear; we could use GAMs and smoothers instead os assuming everything is linear
...
* HTML Slide deck [bit.ly/simpson-summit](https://bit.ly/simpson-summit) &copy; Simpson (2020) [![Creative Commons Licence](https://i.creativecommons.org/l/by/4.0/88x31.png)](http://creativecommons.org/licenses/by/4.0/)
